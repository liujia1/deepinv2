...
- **无噪声**反卷积 PSNR 仅 19.42 dB——并非完美恢复，因为浮点舍入误差经 $1/H(k)$ 放大后已产生可见伪影
- **含噪**反卷积 PSNR 骤降至 4.67 dB——噪声标准差仅 0.01（人眼几乎不可察觉），但经 $1/H(k)$ 放大后重建完全崩溃
...
| 截断反卷积 (ε=1e-3) | 同模型 (IC) | 4.86 |
| 截断反卷积 (ε=1e-3) | 异模型 (非IC) | 4.85 |
| Tikhonov (λ=1e-2) | 同模型 (IC) | 22.34 |
| Tikhonov (λ=1e-2) | 异模型 (非IC) | 21.94 |

#### 可视化结果

![实验1.5不适定性与InverseCrime](配套实验/实验1_5_不适定性与InverseCrime.png)

上排：原始图像、含噪观测、无噪声反卷积、含噪反卷积；下排：Tikhonov(IC)、Tikhonov(非IC)、截断反卷积(IC)、截断反卷积(非IC)。
...
Tikhonov 正则化下，同模型数据 (IC) 重建 PSNR=22.34 dB，异模型数据 (非IC) 为 21.94 dB，差距 0.4 dB。虽然 PSF 宽度仅差 0.3（$\sigma_A=5.0$ vs $\sigma_B=5.3$），IC 数据仍给出了更高的 PSNR——这正是 Inverse Crime 的"虚假优势"。截断反卷积中两者几乎相同（4.86 vs 4.85 dB），因为该方法本身质量太差，IC 效果被噪声淹没。

> **与正文联系**：1.3 节将 Inverse Crime 定义为"用与生成数据相同的模型和数值实现来测试重建算法"。实验表明，IC 问题的严重性取决于重建质量：当重建足够好时（如 Tikhonov），建模误差的影响才会暴露出来。这也说明**避免 Inverse Crime 是验证算法可靠性的基本要求**。

#### Tikhonov 正则化的效果

Tikhonov 将频域除法从 $\hat{y}/H$ 改为 $\bar{H}\hat{y}/(|H|^2 + \lambda)$，分母中的 $\lambda$ 阻止了 $|H(k)| \to 0$ 时的爆炸。含噪数据下 PSNR 从 4.67 dB 提升到 22.34 dB（IC）或 21.94 dB（非IC），提升超过 17 dB。
...
1.4 节的核心论点是"数据项由噪声物理机制唯一决定"。本实验的目标是：

1. **直观对比**相同 PSNR 下三种噪声的视觉差异——这正是"相同度量值可能对应截然不同的视觉质量"的实例
2. **验证数据项匹配的重要性**：用 L² 去噪（均值滤波）处理脉冲噪声 vs L¹ 去噪（中值滤波），后者 PSNR 高 7 dB

### 实验输入

- **原始图像**：Shepp-Logan 幻影（256×256）
- **高斯噪声**：$\sigma = 0.10$
- **Poisson 噪声**：gain = 0.008
- **脉冲噪声**：椒盐噪声，8% 像素被替换
...
| 去噪方法 | PSNR (dB) |
|---|---|
| 脉冲噪声 + L² 去噪（均值滤波） | 20.45 |
| 脉冲噪声 + L¹ 去噪（中值滤波） | **27.83** |

#### 可视化结果

![实验1.9三种噪声对比](配套实验/实验1_9_三种噪声对比_图1B.png)

每行对应一种噪声：含噪图像、残差图、局部放大。

![实验1.9数据项匹配](配套实验/实验1_9_数据项匹配.png)

左：脉冲噪声原图；中：L² 去噪（均值滤波，椒盐残点仍可见）；右：L¹ 去噪（中值滤波，椒盐被有效消除）。

### 结果分析与补充说明

#### 相同 PSNR 下视觉差异巨大

三种噪声的 PSNR 跨度较大（14~31 dB），但即使 PSNR 接近，视觉感受也截然不同——高斯噪声表现为全局细粒度纹理，Poisson 噪声在亮区更粗而暗区几乎不可见，脉冲噪声则是零星的黑白极值点。附录 1A 指出"相同的 MSE 可能对应视觉质量截然不同的图像"，本实验正是这一论断的直观例证。

> **与正文联系**：1.4 节指出数据项的形式由噪声机制唯一决定。实验中三种噪声的空间分布特征完全不同，自然需要不同的惩罚策略：L² 放大大残差（适合高斯的全局小偏差），L¹ 对大残差鲁棒（适合脉冲的稀疏极端偏差），KL 散度对低计数区域更敏感（适合 Poisson 的信号依赖性）。

#### 数据项匹配：L¹ 比 L² 更适合脉冲噪声

脉冲噪声 + 均值滤波（L² 类）PSNR=20.45 dB，中值滤波（L¹ 类）PSNR=27.83 dB，差距达 7.4 dB。均值滤波对极值点的抑制力弱——椒盐噪声的黑白点被"平均"到邻域中，产生灰色污斑；中值滤波直接取中位数，极值点被完全消除。

> **与正文联系**：1.4 节指出 L² 数据项对离群值敏感（平方放大极端残差），L¹ 数据项则鲁棒（绝对值线性增长）。均值滤波是 L² 思想的体现（最小化平方误差），中值滤波是 L¹ 思想的体现（最小化绝对值误差）。实验定量验证了"数据项必须匹配噪声机制"这一核心原则。
...
1.4 节给出统一推导链：噪声分布 → 似然函数 → 负对数似然 → 数据项。本实验的目标是：

1. **验证推导链的数值正确性**：蒙特卡洛采样的噪声分布与理论概率密度精确吻合
2. **直观理解三种数据项的惩罚形状差异**：L² 二次增长、L¹ 线性增长、KL 非对称
3. **理解 L¹ vs L² 的鲁棒性差异**：L¹ 对大残差增长慢，因此对离群值鲁棒

### 实验输入

- **蒙特卡洛采样**：100,000 个样本
- **高斯噪声**：$\mu = 5.0$，$\sigma = 1.0$
- **Poisson 噪声**：$\mu = 20.0$
- **Laplace 噪声**：$\mu = 5.0$，$b = 1.0$
- **数据项对比**：残差范围 $[-5, 5]$，KL 散度中 $y_\text{obs} = 5.0$
...
| 噪声模型 | MC 统计量 | 理论值 | 匹配程度 |
|---|---|---|---|
| 高斯 均值 | 5.0010 | 5.0 | 精确 |
| 高斯 标准差 | 1.0009 | 1.0 | 精确 |
| Poisson 均值 | 19.9921 | 20.0 | 精确 |
| Poisson 方差 | 20.1195 | 20.0 | 精确 |
| Laplace 均值 | 5.0000 | 5.0 | 精确 |
| Laplace 标准差 | 1.4126 | 1.4142 | 精确 |
| KL 最小值位置 | Ax=5.00 | Ax=y=5.0 | 精确 |

#### 可视化结果

![实验1.11似然函数验证](配套实验/experiment_1_11_likelihood.png)

上排：三种噪声的 MC 采样直方图 vs 理论概率密度；下排：三种数据项的惩罚曲线（L²、KL、L¹ vs L² 对比）。

### 结果分析与补充说明

#### 推导链的数值验证

三种噪声的蒙特卡洛统计量（均值、方差/标准差）与理论值精确吻合，验证了 1.4 节推导链的前提——噪声分布的假设是正确的。KL 散度数据项 $D_\text{KL} = Ax - y\ln(Ax)$ 在 $Ax = y = 5.0$ 处取最小值，这正是最大似然估计的结果。

> **与正文联系**：1.4 节的统一推导链"噪声分布 → 似然 → 负对数似然 → 数据项"在实验中得到完整的数值验证。关键洞察：**数据项不是人为选择的，而是由噪声分布唯一决定的**——高斯分布的指数中有 $(y-Ax)^2$，取负对数后得到 L²；Laplace 分布的指数中有 $|y-Ax|$，取负对数后得到 L¹。

#### L¹ vs L² 的鲁棒性差异

L¹ 数据项 $|y-Ax|$ 随残差线性增长，L² 数据项 $(y-Ax)^2/2$ 随残差二次增长。当残差为 5 时，L²=12.5 而 L¹=5——L² 对大残差的惩罚是 L¹ 的 2.5 倍。这意味着在 L² 优化中，少数极端残差（离群值）会主导总误差，迫使解向其靠拢；而 L¹ 中大残差的影响被线性约束，算法可以"忽略"它们。

> **与正文联系**：1.4 节指出"L² 对离群值敏感，L¹ 对离群值鲁棒"。实验的惩罚曲线直观解释了这一论断的数学根源：它来自高斯分布与 Laplace 分布尾部的差异——Laplace 分布的尾部比高斯更"厚"，大偏差出现概率更高，因此对应的数据项对大残差更宽容。这与实验 1.9 中"中值滤波（L¹）比均值滤波（L²）更适合脉冲噪声"的结论完全一致。
