...
4. **验证伴随算子** $A^T$ 的正确性，为后续章节（ULA 采样、梯度下降等）奠定基础
...
- **卷积模糊**：高斯 PSF，$\sigma = 5$ 像素，周期边界条件（循环卷积），通过 FFT 实现正向算子 $A$ 及其伴随 $A^T$
...
**伴随算子验证**（$\langle Ax, y\rangle = \langle x, A^Ty\rangle$，相对差异 $1.58 \times 10^{-15}$）确认了 `blur_adjoint` 的正确性——它在频域使用 $\overline{H(k)}$（共轭）代替 $H(k)$，这正是 $A^T$ 在 DFT 基下的对角表示。伴随算子将在第 3 章（梯度下降）、第 4 章（ULA 采样）中反复使用。
