...
这里 $\text{KL}(q\|p(z|x)) = \mathbb{E}_q[\log(q/p(z|x))]$ 就是 $q$ 和真实后验的 KL 散度。因为 KL 永远 $\geq 0$，所以 $\log p(x) \geq \text{ELBO}(q)$——和 Jensen 一致。但 KL 分解多告诉我们一件事：**ELBO 和 $\log p(x)$ 之间的缝，恰好就是 $\text{KL}(q\|p(z|x))$**。这个缝叫**变分间隙**（variational gap）。
...
直觉：最大化 ELBO = 让模型更认可 $q$ 采的样本 **同时** 让 $q$ 别缩成一个点。熵项防止 $q$ 退化成 $\delta$ 函数（只在一点有质量）。
...
- **重建项** $\mathbb{E}_q[\log p(x|z)]$：给定潜变量 $z$（从 $q$ 采的），模型重建出 $x$ 的对数似然。越大说明 $z$ 把 $x$ 的信息带得越足；
- **KL 正则项** $-\text{KL}(q\|p)$：近似后验 $q$ 偏离先验 $p(z)$ 的程度。越小说明 $q$ 越贴着先验。
...
这种"重建 vs 正则"的拉扯，和第 3 章 MAP 里"数据项 vs 正则项"的拉扯一模一样——8.4 节会深挖这层联系。
...
测试 5 种变分分布 $q$，计算 ELBO 并与 $\log p(x) \approx -1.45$ 对比（注意看最后一行——$q$ 偏到天边，$\text{ELBO}$ 直接跌到 $-50.9$，离天花板差出 $49.5$：**$q$ 离真后验越远，下界越松**，这就是 guesses-the-number 式的直观冲击）：
