...
- **Ground truth / Blurred noisy**：原始图像与含噪模糊观测，展示退化的程度
- **Posterior mean**：后验均值（MMSE估计），视觉效果自然，比MAP估计更平滑
- **Posterior variance**：后验方差图，高方差区域对应模糊导致的信息丢失区域
- **Mean/SD & Coefs of variation**：信噪比型指标，量化重建的相对可靠性
- **Mean/Var coefs (log)**：频域系数的统计，展示不同频率的不确定性分布
