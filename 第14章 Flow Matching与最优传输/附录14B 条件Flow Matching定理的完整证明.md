...
> **定位**：CFM 定理是 Flow Matching 的核心理论结果，其证明需要仔细的积分运算，放入附录以保持正文简洁。如果你接受"梯度等价"这个结论、不想看积分，可以直接跳到末尾"与DSM定理的类比"——那里用一张表点明它和第6章的同构关系。
...
从更深层次看，这个定理回应的是一个看似走投无路的困境：FM 损失要回归的边际向量场 $v_t^*$ 依赖未知的边际密度 $p_t$，训练无从下手；CFM 损失虽可计算，回归的却是"另一个"目标——条件向量场。定理断言：这两条看似岔开的路，在参数空间里迈出的是完全相同的步子。
...
两个证明核心逻辑一致——通过"条件化"把不可算期望变成可算条件期望，靠塔性质证明梯度等价。这正是生成模型的统一模式：**训练时条件化，推理时边际化**。从更深层次看，这张对照表并非巧合的相似，而是同一数学策略在两代范式上的两次应用：第 6 章把"学得分"化归为可算的条件回归，本章把"学向量场"化归为可算的条件回归——困难的期望总在条件化后变得可以估计。

## 更一般的 CFM 定理

上述定理可推广到更一般的条件变量 $z$（不限于 $x_0$）。设 $q(z)$ 是条件变量分布，$p_t(x|z)$ 是条件路径，$v_t(x|z)$ 是条件向量场，则 $\nabla_\theta\mathcal{L}_{\text{FM}}=\nabla_\theta\mathcal{L}_{\text{CFM}}$ 仍成立。这允许：
- $z=x_0$：标准 CFM；
- $z=(x_0,x_1)$（数据-噪声对）：双向条件 FM（用于 OT-CFM）；
- $z=y$（标签/文本）：条件生成 FM（文生图等）。

这一推广正是 14.3 的 OT-CFM（$z$ 取数据-噪声对）与 14.5 的条件生成（$z$ 取文本）能够共用同一套训练逻辑的理论原因：无论条件变量取什么，梯度等价性岿然不动。
