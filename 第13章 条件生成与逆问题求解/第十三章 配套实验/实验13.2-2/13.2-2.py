...
for t_val in t_test_values:
    ...
    print(f"{t_val:5.1f}  {max_err:25.6e}  {'PASS' if max_err < 1 else 'FAIL':>6s}")

print("""
后验得分分解 = 先验得分 + 似然得分 是贝叶斯定理的直接推论
  nabla log p(x_t|y) = nabla log p(y|x_t) + nabla log p(x_t) - nabla log p(y)
  其中 nabla log p(y) = 0（p(y)不依赖x_t）
...
axes[0].axvline(y_obs, color='k', linestyle=':', lw=1, alpha=0.5, label=r'观测 $y={}$'.format(y_obs))
