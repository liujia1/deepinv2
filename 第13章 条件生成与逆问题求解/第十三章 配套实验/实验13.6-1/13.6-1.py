...
def dps_posterior_sample(y_obs, A_func, sigma_y, zeta=1.0, N_particles=5000, N_steps=300, T=1.0, seed=42):
    np.random.seed(seed)
    h = T / N_steps
    ...
    for i in range(N_steps):
        ...
        likelihood_grad = mean_t * (y_obs - Ax0_hat) / sigma_y**2
...
print("\n任务1：去噪 (A=I)")
y_denoise = 0.5
samples_denoise = dps_posterior_sample(y_denoise, lambda x: x, sigma_y=0.3, seed=42)
print(f"  后验采样: 均值={np.mean(samples_denoise):.3f}, 标准差={np.std(samples_denoise):.3f}")

# ---- 任务2：去模糊 (A=0.7x) ----
print("\n任务2：去模糊 (A=0.7x, 下采样比例0.7)")
y_blur = 0.5 * 0.7
samples_blur = dps_posterior_sample(y_blur, lambda x: 0.7 * x, sigma_y=0.3, seed=42)
print(f"  后验采样: 均值={np.mean(samples_blur):.3f}, 标准差={np.std(samples_blur):.3f}")

# ---- 任务3：非线性退化 (A=tanh(x)) ----
print("\n任务3：非线性退化 (A=tanh(2x))")
y_nonlinear = 0.5
samples_nonlinear = dps_posterior_sample(y_nonlinear, lambda x: np.tanh(2 * x), sigma_y=0.3, seed=42)
...
for i in range(n_runs):
    samples = dps_posterior_sample(y_denoise, lambda x: x, sigma_y=0.3, zeta=1.0, seed=i)
