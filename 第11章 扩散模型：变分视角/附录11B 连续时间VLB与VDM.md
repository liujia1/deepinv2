...
>
> 定位一句话：本章主文在**固定调度**下把 VLB 化简成 DDPM，而本附录打开第二个自由度——**连噪声调度一起学**。这个思想延续到第 12 章 12.5 节；此处只把连续时间公式与 VDM 的关键思想讲清。
...
其中 $x_\epsilon$ 是 $t\to 0^+$ 时的含噪状态。形式没变，只是"求和"换成了"沿时间的积分"。这一转变意味着：离散公式里的每一项、每个符号在连续下都有对应物，因此本章所有化简直觉依然成立，只是求和变成了积分、权重由离散点取均匀变成连续权重。
...
而且 VDM 证明：连续时间下三种参数化（噪声/ x₀ / 得分预测）仍等价，且 ε 预测 VLB 的最优权重恰好等于 DSM 的最优权重 $\lambda(\sigma)$——再次强化第12章等价性的结论。从更深层次看：即使在连续时间、连调度都可学的更广设定下，三种参数化依然信息等价、目标一致，说明"殊途同归"不是某个特定工程选择的巧合，而是结构性的。
