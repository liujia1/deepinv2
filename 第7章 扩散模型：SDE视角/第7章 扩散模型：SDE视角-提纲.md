...
> **叙事主线**：为什么需要连续时间(7.1) → 正向SDE：噪声如何加入(7.2) → 逆向SDE：如何从噪声恢复(7.3) → 概率流ODE：确定性等价(7.4) → 数值离散化：如何计算(7.5) → 实践(7.6) → 采样加速：少步生成（训练无关的高阶积分器 + 训练相关的一致性模型）(7.7)
...
**核心观点**：采样本质是"数值求解逆扩散方程"，瓶颈是多次串行的网络前向（NFE）。加速有**两条正交路线**：① **训练无关**——用指数积分器（DPM-Solver 一族、UniPC）把 ODE 离散化做精，在同样 10–20 步内逼近精确解；② **训练相关**——学一个把整条采样路径压成一步/极少数步的映射（一致性模型、CTM、sCM）。两者共享同一源头：7.4 的概率流 ODE 把随机采样变成确定性轨迹，才使得"追轨迹"或"一步映射"成为可能。

- **7.7.1 训练无关：指数积分器 DPM-Solver 一族**
  - 动机：扩散 ODE 的**线性项可闭式解出**，普通 Euler/RK 把它和积分一起近似，白白浪费结构
  - 关键识别：换到 log-SNR 坐标 $\lambda_t=\log(\alpha_t/\sigma_t)$ 后，PF-ODE 精确解只含一个指数加权积分 $\int e^{-\lambda}\epsilon_\theta\,d\lambda$
  - 一阶指数积分器 DPM-Solver-1 = DDIM（DDIM 是"最粗糙的指数积分器"）
  - DPM-Solver++（2阶）关键改进：从预测噪声 $\epsilon_\theta$ 换预测干净数据 $x_\theta$（引导下更稳）
  - 经验规律：DDIM≈50步、DPM-Solver++≈10–20步、DPM-Solver-v3≈5–10步
- **7.7.2 训练相关：把轨迹压成一步的一致性模型**
  - 一致性函数 $f_\theta(x_t,t)\approx x_0$，自一致性（同轨迹所有点给同终点）+ 边界条件（$f_\theta(x,\epsilon)\approx x$）
  - 两种训练：一致性蒸馏 CD（用现成 diffusion 当老师）+ 一致性训练 CT（从头学，改进 EMA、伪 Huber、log-normal 调度）
  - 演进：CM → iCT → CTM（跳多步）→ sCM（15亿参数、ImageNet 512 单步）
- **7.7.3 两族方法对照与选型**：是否需要额外训练、所需 NFE、核心数学、质量上限、适合场景
- **7.7.4 与本书呼应**：衔接 7.5 的 DPM++；呼应第 14 章 Reflow 蒸馏（同为压缩多步→少步）；迁移到第 13 章条件后验采样
- **配套实验 7.7-1**：纯 numpy 2D 四角高斯混合——步骤1 对比 DDIM(1阶) vs DPM-Solver++(2阶) 的步数-负对数似然曲线；步骤2 用解析得分当老师做 CD 蒸馏，对比朴素一步 $x_0$ 预测 / 一致性1步 / 一致性4步 / DDIM 50步

**来源**：Lu et al. (2022) "DPM-Solver / DPM-Solver++"; Lu et al. (2023/2024) "DPM-Solver-v3"; Song et al. (2023) "Consistency Models"; Song & Dhariwal (2023) "iCT"; Kim et al. (2023) "CTM"; Lu & Song (2024) "sCM"; Zhao et al. (2023) "UniPC"; Zhang & Chen (2022) "DEIS"
...
7.6 实践（噪声调度 + 训练 + 采样 + 实验）
      ...
      │ "如何更快？"
      ▼
7.7 采样加速（高阶指数积分器 + 一致性模型）──→ 呼应第14章 Reflow 蒸馏
      │
...
| 高阶求解器（DPM++等） | Lu et al. (2022) | ✅ |
| 指数积分器 DPM-Solver/DPM-Solver++（7.7） | Lu et al. (2022) DPM-Solver / DPM-Solver++ | ✅ |
| DPM-Solver-v3（7.7） | Lu et al. (2023/2024) DPM-Solver-v3 | ✅ |
| 一致性模型 / CD / CT / CTM / sCM（7.7） | Song et al. (2023) CM; Song & Dhariwal (2023) iCT; Kim et al. (2023) CTM; Lu & Song (2024) sCM | ✅ |
| 采样加速两族对照与选型（7.7） | 7.7 正文对照表 | ✅ |
| 采样加速配套实验（实验7.7-1） | 自编 numpy 2D 四角高斯混合 | ✅ |
