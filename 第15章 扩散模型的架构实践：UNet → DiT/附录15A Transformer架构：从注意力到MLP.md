...
第 15.3 节给出的自注意力像图纸上的主干线；而真正把网络“立起来”的，正是下面这些围绕主干的零件——归一化、残差、前馈网、位置编码。把它们组装成一个“块”（Block），就得到了 Transformer 作为先验容器的基本单元。这几件事我们逐件展开。
...
**对比**：Pre-LN 更稳（归一化在子层前，喂给 MHA/MLP 的特征始终归一化）；Post-LN 训得好时可能更快，但需更小心 warmup。DiT 选 Pre-LN——对要注入时间步条件的扩散去噪器，稳定性更重要。从更深层次看，Pre-LN 对条件模型尤其关键：归一化在子层前，让时间步的调制信号能稳定地流过每一层残差主干，这正呼应 15.2.3 节——稳定，是“条件”这种机制得以生长的土壤。
