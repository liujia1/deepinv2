...
## 动手感受一下：最小 UNet 的前向 shape（可运行，只跑形状不训练）

> 下面这段**可运行**代码（依赖 `torch`）不训练任何东西，只验证一件事：编码器-解码器 + 跳跃连接如何做到“输入同尺寸、输出同尺寸”。把它存成 `mini_unet.py`、装好 `torch` 后直接 `python mini_unet.py` 即可看到每层 shape 变化。

```python
# ============================================================
# 最小 UNet 前向 shape 演示（玩具输入，不训练，只跑 shape）
# 依赖: torch。运行: python mini_unet.py
...
class MiniUNet(nn.Module):
    def __init__(self, in_ch=1, base=16):
        super().__init__()
        # 编码器: 两层 conv 保持尺寸(padding=1, kernel=3), 之后 maxpool 减半
    ...
    def forward(self, x):
        show("输入 x", x)
        e1 = self.enc1(x)                 # 尺寸不变, 通道 -> base
        e2 = self.enc2(self.pool(e1))     # 下采样后, 通道 -> 2*base
        d  = self.up(e2)                  # 上采样回 e1 尺寸
        d  = torch.cat([d, e1], dim=1)    # 跳跃连接: 把 e1 细节抄过来(通道相加)
        d  = self.dec1(d)                 # 通道 -> base
        out = self.head(d)                # 通道 -> in_ch, 尺寸同输入
        show("输出 out (尺寸同输入)", out)
        return out
...
跑完你会看到：无论编码器怎么把分辨率减半、解码器怎么加回来，输出尺寸都和输入一样——`[1,1,32,32]` 进、`[1,1,32,32]` 出。这就是“输入同尺寸、输出同尺寸”的保证。
