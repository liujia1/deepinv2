...
results_summary = {
    'scaling_results': {
        name: {
            'params_M': round(float(param_counts[name] / 1e6), 2),
            'final_loss': round(float(trained_histories[name][-1]), 6) if trained_histories.get(name) else None,
        } for name in dit_configs
    },
    'power_law_exponent': round(float(b_fit), 2),
}
