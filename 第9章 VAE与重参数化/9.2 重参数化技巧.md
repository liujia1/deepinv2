...
重参数化这招，表面看只是把 $z\sim q_\phi$ 改写成 $z=\mu+\sigma\epsilon$，实则撬动了整条变分路径：**它让"从随机分布采样"第一次能被反向传播训练**。没有它，ELBO 再漂亮也只是纸上的目标；有了它，神经网络才能去学那个算不出的后验。VAE 能真正跑起来、能训练，全靠这一步。至此，梯度的问题解决了，下一个问题是：ELBO 这个目标到底在优化什么？把它拆开，重建项和 KL 项之间的"拉扯"会塑造整个隐空间——这正是下一节要剖开的。
