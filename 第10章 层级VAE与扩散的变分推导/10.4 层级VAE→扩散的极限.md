...
*图 10.2：左栏为层级 VAE 视角（编码 $q(z_l|z_{l-1})$ 自底向上、解码 $p(z_l|z_{l+1})$ 自顶向下），右栏为扩散模型视角（正向过程 $q(x_t|x_{t-1})$ 逐步加噪、逆向过程 $p_\theta(x_{t-1}|x_t)$ 逐步去噪）。中间桥接表明：在高斯转移 + 编码器固定 + 层数 $T\to\infty$ 下，左栏的编码/解码正是右栏的正向/逆向过程——两者是同一数学对象的不同表述。*
