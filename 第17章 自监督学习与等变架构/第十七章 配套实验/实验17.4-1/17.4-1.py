...
if os.path.exists(CHECKPOINT_SURE):
    checkpoint = torch.load(CHECKPOINT_SURE, map_location=device, weights_only=False)
    ...
    if checkpoint.get('is_final', False):
        print(f"✓ 检测到最终权重，直接加载，跳过训练过程")
        print(f"  训练轮数: {checkpoint['epoch']+1}")
        model_sure.load_state_dict(model_state)
        if optimizer_state is not None:
            optimizer_sure.load_state_dict(optimizer_state)
        train_losses_sure = checkpoint.get('train_losses', checkpoint.get('losses', []))
        residuals_history = checkpoint.get('residuals', [])
        correction_history = checkpoint.get('corrections', [])
        start_epoch_sure = checkpoint['epoch'] + 1
        is_final_sure = True
    else:
...
if not is_final_sure and start_epoch_sure < N_EPOCHS:
    for epoch in range(start_epoch_sure, N_EPOCHS):
        ...
        if (epoch + 1) % 10 == 0 and (epoch + 1) < N_EPOCHS:
            torch.save({
                'epoch': epoch,
                'model_state_dict': model_sure.state_dict(),
                'optimizer_state_dict': optimizer_sure.state_dict(),
                'loss': avg_loss,
                'train_losses': train_losses_sure,
...
start_epoch_naive = 0
is_final_naive = False
train_losses_naive = []

if os.path.exists(CHECKPOINT_NAIVE):
    checkpoint = torch.load(CHECKPOINT_NAIVE, map_location=device, weights_only=False)
    model_state = checkpoint.get('model_state_dict', checkpoint.get('model_state'))
    optimizer_state = checkpoint.get('optimizer_state_dict', checkpoint.get('optimizer_state'))
    ...
    if checkpoint.get('is_final', False):
        ...
        train_losses_naive = checkpoint.get('train_losses', [])
        start_epoch_naive = checkpoint['epoch'] + 1
        is_final_naive = True
    else:
        print(f"检测到未完成的训练，从第 {checkpoint['epoch']+1} 轮继续")
        model_naive.load_state_dict(model_state)
...
if not is_final_naive and start_epoch_naive < N_EPOCHS:
    for epoch in range(start_epoch_naive, N_EPOCHS):
        ...
        if (epoch + 1) % 10 == 0 and (epoch + 1) < N_EPOCHS:
            torch.save({
                ...
                'loss': loss.item(),
                'train_losses': train_losses_naive,
                'is_final': False
            }, CHECKPOINT_NAIVE)
    ...
    torch.save({
        'epoch': N_EPOCHS - 1,
        'model_state_dict': model_naive.state_dict(),
        'loss': train_losses_naive[-1] if train_losses_naive else 0,
        'train_losses': train_losses_naive,
        'is_final': True
    }, CHECKPOINT_NAIVE)
...
if os.path.exists(CHECKPOINT_SUP):
    checkpoint = torch.load(CHECKPOINT_SUP, map_location=device, weights_only=False)
    model_state = checkpoint.get('model_state_dict', checkpoint.get('model_state'))
    optimizer_state = checkpoint.get('optimizer_state_dict', checkpoint.get('optimizer_state'))
    ...
    if checkpoint.get('is_final', False):
        ...
        train_losses_sup = checkpoint.get('train_losses', [])
        start_epoch_sup = checkpoint['epoch'] + 1
        is_final_sup = True
    else:
        print(f"检测到未完成的训练，从第 {checkpoint['epoch']+1} 轮继续")
        model_sup.load_state_dict(model_state)
...
if not is_final_sup and start_epoch_sup < N_EPOCHS:
    ...
    torch.save({
        'epoch': N_EPOCHS - 1,
        'model_state_dict': model_sup.state_dict(),
        'loss': train_losses_sup[-1] if train_losses_sup else 0,
        'train_losses': train_losses_sup,
        'is_final': True
    }, CHECKPOINT_SUP)
...
CHECKPOINT_MC = os.path.join(SAVE_DIR, 'ckpt_MC_Compare.pt')
start_epoch_mc = 0
is_final_mc = False
train_losses_mc = []

if os.path.exists(CHECKPOINT_MC):
    checkpoint = torch.load(CHECKPOINT_MC, map_location=device, weights_only=False)
    model_state = checkpoint.get('model_state_dict', checkpoint.get('model_state'))
    optimizer_state = checkpoint.get('optimizer_state_dict', checkpoint.get('optimizer_state'))
    ...
    if checkpoint.get('is_final', False):
        print(f"✓ 检测到最终权重，直接加载，跳过训练过程")
        print(f"  训练轮数: {checkpoint['epoch']+1}")
        model_mc.load_state_dict(model_state)
        if optimizer_state is not None:
            optimizer_mc.load_state_dict(optimizer_state)
        train_losses_mc = checkpoint.get('train_losses', [])
        start_epoch_mc = checkpoint['epoch'] + 1
        is_final_mc = True
    else:
        print(f"检测到未完成的训练，从第 {checkpoint['epoch']+1} 轮继续")
        model_mc.load_state_dict(model_state)
...
if not is_final_mc and start_epoch_mc < N_EPOCHS:
    for epoch in range(start_epoch_mc, N_EPOCHS):
        model_mc.train()
        pbar = tqdm(train_loader, desc=f'Epoch {epoch+1}/{N_EPOCHS}', leave=False, unit='batch')
        for batch_x, _ in pbar:
            batch_x = batch_x.to(device)
    ...
    torch.save({
        'epoch': N_EPOCHS - 1,
        'model_state_dict': model_mc.state_dict(),
        'loss': train_losses_mc[-1] if train_losses_mc else 0,
        'train_losses': train_losses_mc,
        'is_final': True
    }, CHECKPOINT_MC)
    print(f"      [MC] ✓ 最终checkpoint已保存")
...
start_epoch_autodiff = 0
is_final_autodiff = False
train_losses_autodiff = []

if os.path.exists(CHECKPOINT_AUTODIFF):
    checkpoint = torch.load(CHECKPOINT_AUTODIFF, map_location=device, weights_only=False)
    model_state = checkpoint.get('model_state_dict', checkpoint.get('model_state'))
    optimizer_state = checkpoint.get('optimizer_state_dict', checkpoint.get('optimizer_state'))
    ...
    if checkpoint.get('is_final', False):
        print(f"✓ 检测到最终权重，直接加载，跳过训练过程")
        print(f"  训练轮数: {checkpoint['epoch']+1}")
        model_autodiff.load_state_dict(model_state)
        if optimizer_state is not None:
            optimizer_autodiff.load_state_dict(optimizer_state)
        train_losses_autodiff = checkpoint.get('train_losses', [])
        start_epoch_autodiff = checkpoint['epoch'] + 1
        is_final_autodiff = True
    else:
        print(f"检测到未完成的训练，从第 {checkpoint['epoch']+1} 轮继续")
        model_autodiff.load_state_dict(model_state)
        if optimizer_state is not None:
...
if not is_final_autodiff and start_epoch_autodiff < N_EPOCHS:
    for epoch in range(start_epoch_autodiff, N_EPOCHS):
        ...
        if (epoch + 1) % 10 == 0 and (epoch + 1) < N_EPOCHS:
            torch.save({
                'epoch': epoch,
                'model_state_dict': model_autodiff.state_dict(),
                'optimizer_state_dict': optimizer_autodiff.state_dict(),
                'loss': sure_val.item(),
                'train_losses': train_losses_autodiff,
                'is_final': False
            }, CHECKPOINT_AUTODIFF)
    # 最终checkpoint
    torch.save({
        'epoch': N_EPOCHS - 1,
        'model_state_dict': model_autodiff.state_dict(),
        'loss': train_losses_autodiff[-1] if train_losses_autodiff else 0,
        'train_losses': train_losses_autodiff,
        'is_final': True
    }, CHECKPOINT_AUTODIFF)
...
if os.path.exists(CHECKPOINT_R2R):
    checkpoint = torch.load(CHECKPOINT_R2R, map_location=device, weights_only=False)
    model_state = checkpoint.get('model_state_dict', checkpoint.get('model_state'))
    optimizer_state = checkpoint.get('optimizer_state_dict', checkpoint.get('optimizer_state'))
    ...
    if checkpoint.get('is_final', False):
        print(f"✓ 检测到最终权重，直接加载，跳过训练过程")
        print(f"  训练轮数: {checkpoint['epoch']+1}")
        model_r2r.load_state_dict(model_state)
        if optimizer_state is not None:
            optimizer_r2r.load_state_dict(optimizer_state)
        train_losses_r2r = checkpoint.get('train_losses', checkpoint.get('losses', []))
        start_epoch_r2r = checkpoint['epoch'] + 1
        is_final_r2r = True
    else:
        print(f"检测到未完成的训练，从第 {checkpoint['epoch']+1} 轮继续")
        model_r2r.load_state_dict(model_state)
...
if not is_final_r2r and start_epoch_r2r < N_EPOCHS:
    for epoch in range(start_epoch_r2r, N_EPOCHS):
        ...
        if (epoch + 1) % 10 == 0 and (epoch + 1) < N_EPOCHS:
            torch.save({
                'epoch': epoch,
                'model_state_dict': model_r2r.state_dict(),
                'optimizer_state_dict': optimizer_r2r.state_dict(),
                'loss': avg_loss,
                'train_losses': train_losses_r2r,
                'is_final': False
            }, CHECKPOINT_R2R)
...
for alpha in [0.01, 0.05, 0.1, 0.5, 1.0]:
    ...
    if os.path.exists(a_ckpt_path):
        ...
        ms = ckpt.get('model_state_dict', ckpt.get('model_state'))
        os_ = ckpt.get('optimizer_state_dict', ckpt.get('optimizer_state'))
        model_a.load_state_dict(ms)
        if os_ is not None:
            opt_a.load_state_dict(os_)
        a_start = ckpt['epoch'] + 1
        is_final_a = ckpt.get('is_final', False)
...
if os.path.exists(CHECKPOINT_BS):
    checkpoint = torch.load(CHECKPOINT_BS, map_location=device, weights_only=False)
    model_state = checkpoint.get('model_state_dict', checkpoint.get('model_state'))
    optimizer_state = checkpoint.get('optimizer_state_dict', checkpoint.get('optimizer_state'))
    ...
    if checkpoint.get('is_final', False):
        print(f"✓ 检测到最终权重，直接加载，跳过训练过程")
        print(f"  训练轮数: {checkpoint['epoch']+1}")
        model_bs.load_state_dict(model_state)
        if optimizer_state is not None:
            optimizer_bs.load_state_dict(optimizer_state)
        train_losses_bs = checkpoint.get('train_losses', checkpoint.get('losses', []))
        start_epoch_bs = checkpoint['epoch'] + 1
        is_final_bs = True
    else:
        print(f"检测到未完成的训练，从第 {checkpoint['epoch']+1} 轮继续")
        model_bs.load_state_dict(model_state)
...
if not is_final_bs and start_epoch_bs < N_EPOCHS:
    for epoch in range(start_epoch_bs, N_EPOCHS):
        ...
        if (epoch + 1) % 10 == 0 and (epoch + 1) < N_EPOCHS:
            torch.save({
                'epoch': epoch,
                'model_state_dict': model_bs.state_dict(),
                'optimizer_state_dict': optimizer_bs.state_dict(),
                'loss': avg_loss,
...
def train_gaussian_denoiser(sigma, base=16, epochs=20):
    ...
    if os.path.exists(ckpt_path):
        ...
        if checkpoint.get('is_final', False):
            ...
            train_losses_g = checkpoint.get('train_losses', [])
            start_epoch_g = checkpoint['epoch'] + 1
            is_final_g = True
        else:
            print(f"检测到未完成的训练，从第 {checkpoint['epoch']+1} 轮继续")
            model.load_state_dict(model_state)
    ...
    torch.save({
        'epoch': epochs - 1,
        'model_state_dict': model.state_dict(),
        'loss': train_losses_g[-1] if train_losses_g else 0,
        'train_losses': train_losses_g,
        'is_final': True
    }, ckpt_path)
