...
**一句话直觉**：两边展开后，只有交叉项不同；而交叉项里，"对加噪图求边际得分"与"给定干净图求条件得分、再对干净图平均"其实是同一件事（这就是 Tweedie 等式的精神）。所以两个目标只差一个与网络参数无关的常数 $C$——优化它们得到的解完全相同。这一结论至关重要：它意味着"去噪即学得分"不是巧合，而是必然。
...
两者的差别一眼可见：ESM 用边际得分 $\nabla\log q_\sigma(\tilde x)$ 作监督，而 DSM 用条件得分 $\nabla\log q_\sigma(\tilde x|x)$ 作监督——后者是闭式已知的。所谓等价性，就是要证明这种"监督信号的替换"不改变优化结果。
...
第一项两边相同（注意 $\mathbb{E}_{q_\sigma(\tilde x)}=\mathbb{E}_{q_\sigma(\tilde x,x)}$，因为后者对 $x$ 的边际就是前者），所以**关键就在比交叉项**。
...
**ESM 的交叉项 = DSM 的交叉项！** 这就是整件事的命门——也再次印证了我们开头的那句直觉：对 $x$ 平均后的边际得分，与被直接当做监督的条件得分，在期望意义上是同一回事。
...
3. **$\sigma\to0$ 的极限**：此时 $q_\sigma(\tilde x)\to p(\tilde x)$，$q_\sigma(\tilde x|x)\to\delta(\tilde x-x)$，$C(\sigma)\to0$，DSM 与 ESM 完全一致。这一极限也解释了 6.3 节"$\sigma$ 足够小时 $s_{\theta^*}\approx\nabla\log p$"的说法，并为 6.5 节"$\sigma$ 太小会有低密度区问题"埋下伏笔。
