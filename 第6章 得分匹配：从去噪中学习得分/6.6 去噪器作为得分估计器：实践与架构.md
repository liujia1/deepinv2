...
UNet（Ronneberger et al. 2015）是图像去噪和得分估计的事实标准架构，其核心设计包括：

- **编码器**：逐步下采样，提取多尺度特征
- **解码器**：逐步上采样，恢复空间分辨率
- **跳跃连接**：将编码器的特征直接传递给解码器对应层，保留空间细节
- **残差块**：每个卷积块内使用残差连接，改善梯度流动

对于图像去噪任务，UNet的优势在于：

- 多尺度特征提取：大尺度特征捕获全局结构，小尺度特征捕获局部纹理
- 跳跃连接保留高频细节，避免解码器需要从低分辨率恢复所有信息
- 参数效率高：相比同等感受野的全卷积网络，参数更少
...
**多噪声水平训练时**：推荐噪声预测（$\varepsilon$-prediction），因为预测目标的量级与 $\sigma$ 无关，训练最稳定。

**逆问题求解时**：信号预测（$x_0$-prediction）更直观，因为输出直接是去噪结果，可以直接插入PnP框架。

**理论分析时**：得分预测（$s$-prediction）最自然，因为它直接对应Langevin方程的驱动力。

三种参数化之间可以自由转换——训练时用一种，推理时用另一种：

$$s_\theta = -\frac{\varepsilon_\theta}{\sigma} = \frac{D_\theta - \tilde{x}}{\sigma^2}$$

$$\varepsilon_\theta = -\sigma\,s_\theta = \frac{\tilde{x} - D_\theta}{\sigma}$$

$$D_\theta = \tilde{x} + \sigma^2 s_\theta = \tilde{x} - \sigma\varepsilon_\theta$$
