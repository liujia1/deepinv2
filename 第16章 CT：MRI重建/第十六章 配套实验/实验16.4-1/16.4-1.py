...
if start_epoch_unet >= n_epochs_unet:
    ...
    for epoch in range(start_epoch_unet, n_epochs_unet):
        epoch_loss = 0
        n_batches = 0
        pbar = tqdm(loader, desc=f'UNet Epoch {epoch+1}/{n_epochs_unet}', ncols=100, leave=True)
...
if start_epoch_lgd >= n_epochs_lgd:
    ...
    for epoch in range(start_epoch_lgd, n_epochs_lgd):
        epoch_loss = 0
        n_batches = 0
        pbar = tqdm(loader, desc=f'LGD Epoch {epoch+1}/{n_epochs_lgd}', ncols=100, leave=True)
        for batch_x, _ in pbar:
...
lgd_alphas = [round(block.alpha.item(), 4) for block in lgd.blocks]

results_summary = {
    "experiment": "实验16.4-1 学习型重建——UNet后处理与算法展开",
    "步骤1_UNet训练": {
        "最终Loss": round(float(unet_losses[-1]), 6) if unet_losses else None,
        "训练轮数": n_epochs_unet,
    },
    "步骤2_LGD训练": {
        "最终Loss": round(float(lgd_losses[-1]), 6) if lgd_losses else None,
        "训练轮数": n_epochs_lgd,
        "学习步长alpha": lgd_alphas,
    },
    "参数量对比": {
        "UNet": n_params_unet,
        "LGD每块": n_params_lgd_per_block,
        "LGD总计_K5": n_params_lgd,
        "LGD总计除UNet": round(n_params_lgd / n_params_unet, 1),
    },
    "步骤3_方法对比_PSNR_dB": {
        "零填充": {"mean": round(float(np.mean(psnr_zf)), 2), "std": round(float(np.std(psnr_zf)), 2)},
        "UNet": {"mean": round(float(np.mean(psnr_unet)), 2), "std": round(float(np.std(psnr_unet)), 2)},
        "LGD": {"mean": round(float(np.mean(psnr_lgd)), 2), "std": round(float(np.std(psnr_lgd)), 2)},
    },
}
