...
> **过渡**：FBP在全角、低噪声条件下工作良好，但当投影角度不完整或噪声较大时，CT重建变得不适定——这正是第1章讨论的逆问题一般性困难在CT中的具体表现。
...
> **过渡**：CT的正则化重建展示了"先验选择决定重建质量"这一核心原则。现在我们转向另一种医学成像模态——MRI，它的正向模型与CT完全不同（傅里叶采样而非线积分），但不适定性的挑战同样存在，只是形式不同。
...
> **过渡**：学习型方法用数据替代了手工先验，但仍有两个根本局限：(1) 训练需要配对数据；(2) 重建结果是点估计，无法量化不确定性。扩散先验方法同时解决了这两个问题——这正是第13章"条件扩散=逆问题求解"在医学成像中的落地。
