...
可以写成紧凑的矩阵形式（把"旋转 + 弛豫"合并成一个线性系统加常数项）：
...
在没有弛豫项时（$T_1,T_2 \to \infty$），磁化矢量绕 $z$ 轴做 Larmor 进动（Larmor precession，像陀螺绕轴转）：

$$\mathbf{m}(t) = R_z(\omega_0 t) \mathbf{m}(0)$$

其中 $\omega_0 = \gamma B_0$ 是 Larmor 频率，$R_z(\phi)$ 是绕 $z$ 轴转 $\phi$ 角的旋转矩阵：
...
**纵向恢复**（纵向磁化向平衡值 $m_0$ 指数趋近）：

$$m_z(t) = m_0 + (m_z(0) - m_0) e^{-t/T_1}$$

关键观察：$T_1$ 恢复是向 $m_0$ 的指数趋近，$T_2$ 衰减是向零的指数衰减。对生物组织，通常 $T_1 > T_2$（如脑组织 $T_1 \sim 600\text{ms}$，$T_2 \sim 100\text{ms}$）。

## B.4 稳态信号

在稳态（$\frac{d\mathbf{m}}{dt} = 0$）下求解 Bloch 方程，设横向场 $B_y$ 为 RF 激发产生的场：

$$m_{xy}^{ss} = \frac{T_2 \gamma B_y}{T_1 T_2 (\gamma B_y)^2 + 1} \cdot m_0$$

这个稳态信号的大小取决于 $B_y$——通过调 $B_y$ 的大小，能改变不同组织间的信号对比度，这正是 MRI 对比度机制的数学基础。

## B.5 从Bloch方程到信号方程

在梯度场 $B_z = \mathbf{g}(t)\cdot\mathbf{r}$ 作用下，不同位置的质子以不同频率进动。横向磁化为：

$$m_{xy}(t,\mathbf{r}) = u(\mathbf{r}) \exp\left(-i\gamma \int_0^t \mathbf{g}(\tau)\cdot\mathbf{r}\, d\tau\right)$$

测量信号为所有位置横向磁化的体积分（Faraday 感应定律）：
...
定义 k-space 变量 $\mathbf{k}(t) = \frac{\gamma}{2\pi} \int_0^t \mathbf{g}(\tau) d\tau$，则：
