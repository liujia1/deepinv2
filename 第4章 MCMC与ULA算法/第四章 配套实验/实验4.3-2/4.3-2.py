...
results_summary = {
    'experiment': '4.3-2',
    'title': 'ULA步长delta对收敛的影响',
    'step1_stepsize_effect': {
        'delta_list': [r[0] for r in results],
        'empirical_vars': [float(round(r[1], 6)) if r[1] != float('inf') else 'inf' for r in results],
        'theoretical_vars': [float(round(r[2], 6)) if r[2] != float('inf') else 'inf' for r in results],
        'empirical_means': [float(round(r[3], 4)) if not np.isnan(r[3]) else 'nan' for r in results],
        'status': [r[4] for r in results],  # converged / diverged
    }
}
