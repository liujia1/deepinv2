...
> "ULA 很美，可它怕尖角——一旦后验里出现 TV 范数那种'带尖儿'的不可微项，梯度就断片了。怎么办？把尖角磨圆。"

4.3 节我们知道 ULA 快，但有个硬前提：$\nabla\log p(x|y)$ 得存在、还得 Lipschitz 连续。可图像逆问题的后验常常长这样：$p(x|y)\propto\exp\{-f(x)-g(x)\}$，其中 $g(x)$ 是 TV 范数、$\ell_1$、指示函数这类**不可微**项（带着尖角）。梯度在尖角处直接"断片"，ULA 用不了。本节讲 MYULA（Proximal ULA，近端朗之万算法）：用 Moreau-Yoshida 包络（一种光滑化技巧）把尖角磨平，在磨平后的近似分布上跑 ULA——误差可控、还有非渐近保证。

**本节目标**（读完后你应该能）：
- 说清"不可微后验"为什么卡住 ULA
- 用大白话理解 Moreau-Yoshida 包络怎么把尖角磨圆，并写出它的梯度（用近端算子）
...
函数 $g$ 的 **Moreau-Yoshida 包络**（Moreau 包络）定义为：

$$g_\lambda(x) = \inf_{u} \left\{g(u) + \frac{1}{2\lambda}\|u - x\|^2\right\}$$

它和第3章的**近端算子**（proximal operator，离 $x$ 最近、又让 $g$ 小的那个点）是一对孪生兄弟：

$$\text{prox}_{\lambda g}(x) = \arg\min_{u} \left\{g(u) + \frac{1}{2\lambda}\|u - x\|^2\right\}$$

也就是——使 $g_\lambda(x)$ 取到下确界的那个 $u$，就是 $\text{prox}_{\lambda g}(x)$。包络给"最小值"，近端算子给"取到最小值的点"。**用大白话讲**：Moreau 包络就是"在 $x$ 附近、给 $g$ 加了个软枕头的条件下，把 $g$ 磨平滑后的版本"。
...
**直觉例子**（三个经典尖角怎么被磨圆）：
- **Laplace 先验** $\exp(-|x|)$：$|x|$ 在 0 处有个尖角，$g_\lambda$ 把尖角磨圆，$\lambda$ 越大磨得越狠。
...
> **权衡先说结论**：$\lambda$ 越小，$p_\lambda$ 越贴近 $p$，但 $L=L_f+1/\lambda$ 越大 → ULA 步长 $\delta$ 必须越小 → 收敛越慢。**精度 vs 速度，天生一对冤家。**
