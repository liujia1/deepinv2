...
## 📌 实验3.5-1：2x2像素TV最小化 —— TV目标函数的几何直觉
...
运行 `3.5-1.py` 后，生成一张包含4个子图的搜索方法对比图。

![2x2 TV最小化搜索对比](./配套实验/实验3.5-1/步骤1_2x2_TV搜索对比.png)
...
CP的三步公式并非凭空给出，而是鞍点问题上近端梯度法的自然结果。下面展示推导逻辑。

**对偶步推导**：固定 $x$，对偶问题为 $\sup_y \langle Kx, y\rangle - g^*(y)$。对 $-g^*(y)$ 做一步近端梯度上升（步长 $\sigma$）：

$$y_{k+1} = \text{prox}_{\sigma g^*}\big(y_k + \sigma \nabla_y[\langle K\bar{x}_k, y\rangle]\big) = \text{prox}_{\sigma g^*}(y_k + \sigma K\bar{x}_k)$$

这里用外推点 $\bar{x}_k$ 替代 $x_k$——这是CP的关键设计，外推使对偶更新"看到"原始变量的前瞻方向，而非滞后反馈。

**原始步推导**：固定 $y$，原始问题为 $\min_x f(x) + \langle Kx, y\rangle$。对 $f(x)$ 做一步近端梯度下降（步长 $\tau$），线性项 $\langle Kx, y\rangle = x^\top K^\top y$ 的梯度为 $K^\top y$：

$$x_{k+1} = \text{prox}_{\tau f}\big(x_k - \tau \nabla_x[\langle Kx, y_{k+1}\rangle]\big) = \text{prox}_{\tau f}(x_k - \tau K^\top y_{k+1})$$

注意使用的是 $y_{k+1}$（已更新的对偶变量），形成Gauss-Seidel式的交替更新。

**外推步**：$\bar{x}_{k+1} = 2x_{k+1} - x_k$ 是简单的线性外推，使下一轮对偶步基于"原始变量将要到达的位置"而非"原始变量当前所在的位置"，加速原始-对偶之间的信息传递。
...
原始步中，$\text{prox}_{\tau f}(v) = \arg\min_x \frac{1}{2\tau}\|x-v\|^2 + \frac{1}{2}\|x-b\|^2$。对 $x$ 求梯度令其为零：$\frac{1}{\tau}(x-v) + (x-b) = 0$，解得 $x = \frac{v + \tau b}{1 + \tau}$。代入 $v = x_k - \tau D^\top y_{k+1}$：
...
其中 $\text{prox}_{\tau f}(v) = \arg\min_x \frac{1}{2\tau}\|x-v\|^2 + \frac{1}{2}\|Ax-b\|^2$。对 $x$ 求梯度令其为零：$\frac{1}{\tau}(x-v) + A^\top(Ax-b) = 0$，整理得 $(I + \tau A^\top A)x = v + \tau A^\top b$，因此 $\text{prox}_{\tau f}(v) = (I + \tau A^\top A)^{-1}(v + \tau A^\top b)$。代入 $v = x_k - \tau D^\top y_{k+1}$ 得展开式：
...
**步长条件的来源**：$\tau\sigma\|K\|^2 < 1$ 保证原始与对偶更新组成的算子是非扩张的（nonexpansive），从而迭代不会发散。直觉上，$\tau$ 和 $\sigma$ 分别控制原始步和对偶步的步长，而 $\|K\|^2$ 衡量了原始-对偶耦合的强度——耦合越强，步长必须越小以保持稳定性。

**实用取值**：对二维离散梯度算子 $D$，谱范数上界为 $\|D\|^2 \leq \|D_h\|^2 + \|D_v\|^2 = 4 + 4 = 8$。因此常取 $\tau = \sigma = 1/\sqrt{8} \approx 0.354$，此时 $\tau\sigma\|D\|^2 \leq 1$。实践中也可以通过幂迭代精确估计 $\|D\|$，取更大的步长加速收敛。

---

## 📌 实验3.5-2：Chambolle-Pock算法——TV去噪与保边性验证
...
运行 `3.5-2.py` 后，生成一张包含5个子图的综合对比图。

![实验3.5-2 TV去噪对比](./配套实验/实验3.5-2/步骤1_TV去噪对比.png)
...
$\rho$ 控制约束 $Kx = z$ 的惩罚力度，其选取直接影响收敛速度和数值稳定性。

**量纲直觉**：$\rho$ 的量纲应与 $f$ 的Hessian的特征值量级一致。对 $f(x) = \frac{1}{2}\|Ax - b\|^2$，Hessian为 $A^\top A$，一个经验起点是 $\rho \sim \|A^\top A\|$ 的某个分位数，或简单取 $\rho = 1$（各变量归一化后）。

**$\rho$ 过小**：惩罚力度弱，约束 $Kx \approx z$ 满足缓慢，x步和z步解耦严重，收敛慢。

**$\rho$ 过大**：惩罚项主导目标，x步的线性系统条件数恶化（$A^\top A$ 被 $\rho D^\top D$ 淹没），迭代震荡。

**自适应策略**（Boyd et al., 2011）：根据原始残差 $r^k = Kx^{k+1} - z^{k+1}$ 和对偶残差 $s^k = \rho K^\top(z^{k+1} - z^k)$ 的比例动态调整——若 $\|r^k\| > \mu\|s^k\|$ 则增大 $\rho$，反之减小，保持两个残差量级均衡。

### x步推导：从增广拉格朗日到线性系统

x步的目标是固定 $z_k, u_k$，求解：

$$\min_x \, f(x) + \frac{\rho}{2}\|Kx - z_k + u_k\|_2^2$$

对TV正则化，$f(x) = \frac{1}{2}\|Ax - b\|^2$，$K = D$，这是关于 $x$ 的无约束二次问题。对 $x$ 求梯度并令其为零：

$$A^\top(Ax - b) + \rho D^\top(Dx - z_k + u_k) = 0$$

整理得到线性系统：

$$(A^\top A + \rho D^\top D)\, x = A^\top b + \rho D^\top(z_k - u_k)$$

当 $A^\top A + \rho D^\top D$ 可逆时，x步有唯一解。这一步只有三行代数，但它是增广拉格朗日→线性系统的关键环节——补上后，从目标函数到可执行算法的推导链就完整了。
...
z步的推导：固定 $x_{k+1}$ 和 $u_k$，z步目标为 $\min_z \, \lambda\|z\|_1 + \frac{\rho}{2}\|Dx_{k+1} - z + u_k\|^2$。这正是近端算子的定义——$\text{prox}_{(\lambda/\rho)\|\cdot\|_1}(Dx_{k+1} + u_k)$，因此 $z_{k+1} = \mathcal{S}_{\lambda/\rho}(Dx_{k+1} + u_k)$，即以阈值 $\lambda/\rho$ 做软阈值。
...
| 收敛条件 | $f, g$ 凸、proper、l.s.c.，Slater条件满足，$\rho > 0$ | $\tau\sigma\|K\|^2 < 1$，$f, g^*$ 凸、proper、l.s.c. |
| 收敛率 | $O(1/k)$（残差范数） | $O(1/k)$（平均迭代） |
| 每步代价 | 求解线性系统 + 近端 | 近端 + 矩阵向量乘 |
| 适用场景 | 有自然分裂的问题 | 标准鞍点结构 |

**ADMM收敛条件**：当 $f$ 和 $g$ 均为proper、convex、lower semicontinuous，且问题有解（Slater条件满足，即存在 $x^*$ 使得 $Kx^* \in \text{dom}\, g$）时，对任意 $\rho > 0$，ADMM迭代保证原始残差 $r^k = Kx^{k+1} - z^{k+1}$ 和对偶残差 $s^k = \rho K^\top(z^{k+1} - z^k)$ 均收敛到零，目标函数值收敛到最优值。
