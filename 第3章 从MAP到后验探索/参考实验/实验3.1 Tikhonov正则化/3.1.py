...
# ---- 4. Tikhonov正则化：高斯噪声 + 高斯先验 → 闭式解 ----
# 3.3节：λ = σ²/σ_x²（正则化参数的概率含义）
# 先验: x ~ N(0, σ_x² I)
# 似然: y|x ~ N(Ax, σ² I)
...
def tikhonov_gradient(x_vec, A, y, lam):
    """3.2节：梯度 x_{k+1} = x_k - τ[A'(Ax_k-y) + λx_k]"""
...
axes[0, 2].set_title(f'Tikhonov闭式解\nλ={lam:.4f}, PSNR={psnr_closed:.1f}dB')
axes[0, 2].axis('off')
