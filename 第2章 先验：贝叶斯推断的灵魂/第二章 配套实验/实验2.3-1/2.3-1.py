...
results_summary = {
    'num_samples': N,
    'noise_sigma': float(sigma_eps),
    'lambda_tikhonov_optimal': float(round(lambda_tikh_optimal, 6)),
    'lambda_lasso': float(lambda_lasso),
    'nrmse_identity_pct': float(round(err_identity * 100, 2)),
    'nrmse_tikhonov_pct': float(round(err_tikh * 100, 2)),
    'nrmse_lasso_pct': float(round(err_lasso * 100, 2)),
    'nrmse_bayes_pct': float(round(err_bayes * 100, 2)),
    'ess_mean': float(round(ess_mean, 2)),
    'ess_min': float(round(ess_min, 2)),
    'low_ess_count': int(low_ess_count),
    'ess_threshold': float(round(ESS_THRESHOLD, 2)),
}
